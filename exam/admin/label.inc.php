...
if($action=='mod'){
	...
	$url2 = '$webdb[www_url]/exam/exam_write.php?fid=$fid&id=$id';
	$url1 = '$url';
	...
	$url2 = '$webdb[www_url]/exam/list.php?fid=$fid';
	$url1 = '$list_url';
	$postdb[tplpart_1code]=str_replace($url1,$url2,StripSlashes($tplpart_1));
	$postdb[tplpart_2code]=str_replace($url1,$url2,StripSlashes($tplpart_2));
	...
	$SQL="SELECT A.*,A.name AS title,A.creattime AS posttime,B.name AS fname FROM {$_pre}form A LEFT JOIN {$_pre}sort B ON A.fid=B.fid $SQL ORDER BY $order $asc LIMIT $sqlmin,$rows ";
